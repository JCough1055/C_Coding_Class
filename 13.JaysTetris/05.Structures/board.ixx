...
//Function that looks to see if a row is complete and returns a true or flase
...
//This function moves all rows down
 ...
 int clear_complete_lines(){
        ...
        for(int y=HEIGHT-1;y>=0;--y){
            if(is_line_complete(y)){
                clear_line(y);
                ++lines_cleared;
                ++y; //Recheck the same row after clearing
            }
        }
 ...
 bool is_game_over(const Tetromino& tetromino)const{
    //If a newly spawned piece immediately collides, the game is over
